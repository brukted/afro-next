...
void main() {
  test('compiler lowers arithmetic graphs into deterministic GLSL', () {
    ...
    expect(
      result.compiledFunction!.topologicalNodeIds,
      [updatedInput.id, updatedConstant.id, add.id, output.id],
    );
    expect(
  ...
  test('compiler rejects cycles instead of silently appending nodes', () {
    ...
    expect(
      result.diagnostics.any((diagnostic) => diagnostic.code == 'cycle_detected'),
      isTrue,
    );
  ...
  test('compiler reports type mismatches on invalid links', () {
    ...
    expect(result.hasErrors, isTrue);
    expect(
      result.diagnostics.any((diagnostic) => diagnostic.code == 'type_mismatch'),
      isTrue,
    );
  });
  ...
  test('compiler lowers sampler and local-variable nodes', () {
    ...
    expect(
      result.compiledFunction!.source,
      contains('float t0 = dot(texture(sampler_2, pos).rgb, vec3(0.299, 0.587, 0.114));'),
    );
